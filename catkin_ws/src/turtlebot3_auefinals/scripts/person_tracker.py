...
def callback(data):
    ...
    if len(data.people)!=0:
        # man_x = data.poses[0].position.x
        # man_y = data.poses[0].position.y
        # man_z = data.poses[0].position.z
        
        man_x = data.people[0].pos.x
        man_y = data.people[0].pos.y
        man_z = data.people[0].pos.z 

        # distance = sqrt((man_x - tb_x_trans)**2 + (man_y - tb_y_trans)**2) 
        ...
        # distance_angular = yaw_tb - yaw_man
        ...
        if error_x>0.0:
            ...
            print("x vel",vel_msg.linear.x)
        elif error_x<=0.0:
            vel_msg.linear.x = 0.0
            print("xstop")
        if error_z>-0.7:
            vel_msg.angular.z = error_z*0.5
...
def main():
    ...
    # rospy.Subscriber('/to_pose_array/leg_detector',PoseArray,callback)
